...
// 시군구 전체 (최신 주 스냅샷)
const REGION_SIGUNGU_SQL = () =>
  `SELECT sido_nm, sigungu_nm,
    SUM(ypx_revise_subscriber_cnt+ypxn_revise_subscriber_cnt+ypxt_revise_subscriber_cnt) as ypx_sub,
    SUM(ypx_order_cnt) as ord
  FROM \`ygy-datawarehouse.report.yogiyo_weekly_region_subscription_ypx\`
  WHERE week_last_date = (SELECT MAX(week_last_date) FROM \`ygy-datawarehouse.report.yogiyo_weekly_region_subscription_ypx\`)
...
const CPS_YOGITHE_ORDER_SQL = (afterDate, daily = false) =>
  `SELECT ${daily ? 'DATE(p.order_dt)' : 'DATE_ADD(DATE_TRUNC(DATE(p.order_dt), WEEK(MONDAY)), INTERVAL 6 DAY)'} as date,
    COUNT(*) as order_cnt
...
// CPS 입점 매장 수 SQL — 일별 유니크 vendor_id (yogithe vs 일반)
const CPS_VENDOR_COUNT_SQL = (afterDate) =>
  `SELECT event_date as date,
    CASE WHEN page_id = '/yogithe_home' THEN 'yogithe' ELSE 'general' END as channel,
    COUNT(DISTINCT vendor_id) as vendor_cnt
  FROM \`ygy-datawarehouse.edw.lst_ilog_event\`
  WHERE event_date > '${afterDate}'
    AND event_date < CURRENT_DATE('+09:00')
    AND page_action = 'click.list.vendor'
...
// CPS 매장 카테고리 분포 SQL — 최근 7일 기준 TOP 카테고리
const CPS_CATEGORY_SQL = (afterDate) =>
  `WITH cps_vendors AS (
    SELECT DISTINCT vendor_id,
      CASE WHEN page_id = '/yogithe_home' THEN 'yogithe' ELSE 'general' END as channel
    FROM \`ygy-datawarehouse.edw.lst_ilog_event\`
    WHERE event_date > DATE_SUB(CURRENT_DATE('+09:00'), INTERVAL 7 DAY)
      AND page_action = 'click.list.vendor'
      AND vendor_ad_id IS NOT NULL AND vendor_ad_id > 0
  )
  SELECT v.channel, cat, COUNT(DISTINCT v.vendor_id) as cnt
  FROM cps_vendors v
  LEFT JOIN \`ygy-datawarehouse.mart.fact_daily_vendor_id\` d ON v.vendor_id = d.vendor_id
    AND d.base_date = DATE_SUB(CURRENT_DATE('+09:00'), INTERVAL 1 DAY)
  , UNNEST(IF(ARRAY_LENGTH(d.vms_category_list) > 0, d.vms_category_list, ['미분류'])) as cat
...
const MEMBERSHIP_SQL = (afterDate) =>
  `SELECT base_date as date,
    SUM(CASE WHEN subscribe_product_cd = 'yogipass' THEN subscriber_cnt ELSE 0 END) as classic,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassxn' THEN subscriber_cnt ELSE 0 END) as naver,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassxt' THEN subscriber_cnt ELSE 0 END) as toss,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassx' THEN subscriber_cnt ELSE 0 END) as direct_ypx,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassxn' THEN \`in\`.new_subscriber_cnt ELSE 0 END) as new_naver,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassxt' THEN \`in\`.new_subscriber_cnt ELSE 0 END) as new_toss,
    SUM(CASE WHEN subscribe_product_cd = 'yogipassx' THEN \`in\`.new_subscriber_cnt ELSE 0 END) as new_direct,
    SUM(CASE WHEN subscribe_product_cd = 'yogipass' THEN \`in\`.new_subscriber_cnt ELSE 0 END) as new_classic
  FROM \`ygy-datawarehouse.mart.fact_daily_customer_subscribe\`
  WHERE base_date > '${afterDate}' AND base_date < CURRENT_DATE()
    AND subscribe_product_cd IN ('yogipassxn','yogipassxt','yogipassx','yogipass')
  GROUP BY 1 ORDER BY 1`;
...
function MembershipContent({ chartData, checked, refreshStatus, onRefresh, range }) {
  ...
  // 구독자: 주간 데이터만 있는 행 사용 (일별 행에서 NaN 방지)
  const subQtyData = filteredSubData.map(r => {
  ...
  return (
    <>
      ...
      {activeSeries.length === 0 ? (
        ...
        <>
          {/* 1. 신규 구독자 추이 — 일별 신규 가입 (맨 위) */}
          {subSeries.length > 0 && (() => {
            const newSubData = filteredSubData.map(r => {
              const row = { date: dateLabel(r.date) };
              subSeries.forEach(s => {
                row[s.id] = s.id === "sub_naver" ? (r.new_naver || 0) : s.id === "sub_toss" ? (r.new_toss || 0) : s.id === "sub_direct" ? (r.new_direct || 0) : (r.new_classic || 0);
              });
              return row;
            });
            return newSubData.length > 0 ? (
              <ChartCard
                ...
                tooltipFormatter={(v, id) => { const s = ALL_SERIES.find(x => x.id === id); return [(+v).toLocaleString("ko-KR") + "명", (s?.label || "") + " 신규"]; }}
                height={240}
              />
            ) : null;
          })()}
...
function RegionContent({ regionData, regionOrdData, sigunguData, regionLoaded, refreshStatus, onRefresh, range }) {
  ...
  // 시군구 드릴다운 — 미리 로드된 sigunguData에서 프론트 필터
  const drillData = drillSido && sigunguData.length
    ? sigunguData.filter(r => r.sido_nm === drillSido).sort((a, b) => (+b.ord || 0) - (+a.ord || 0)).slice(0, 20)
    : [];

  function loadDrill(sido) {
    setDrillSido(drillSido === sido ? null : sido);
  }
        ...
        {kpis.map(k => {
          ...
          return (
            <div key={k.label} style={{ flex: "1 1 80px", background: "white", borderRadius: 10, padding: "10px 12px", boxShadow: "0 1px 4px rgba(0,0,0,0.07)", cursor: isDrillable ? "pointer" : "default", border: isOpen ? "1.5px solid " + k.color : "1.5px solid transparent", transition: "border 0.15s" }}
              onClick={() => isDrillable && loadDrill(k.sido)}>
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {[
          ...
          { label: "일반 GMV", val: (filteredCps.reduce((s,r) => s + (r.gen_gmv||0), 0) / 100000000).toFixed(1) + "억", color: "#3498db" },
          { label: "yogithe GMV", val: (filteredCps.reduce((s,r) => s + (r.yogi_gmv||0), 0) / 100000000).toFixed(1) + "억", color: "#e74c3c" },
        ].map(k => (
      ...
      {/* 총 주문금액 (GMV) 비교 차트 */}
      <div style={{ background: "white", borderRadius: 10, padding: "16px", marginBottom: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#444", marginBottom: 12 }}>CPS 총 주문금액 (일별)</div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={cvrChartData} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="date" tick={{ fontSize: 9 }} interval={xInterval(cvrChartData.length)} />
            <YAxis tickFormatter={v => (v / 100000000).toFixed(1) + "억"} tick={{ fontSize: 9 }} width={48} />
            <Tooltip formatter={(v, name) => [(+v).toLocaleString("ko-KR") + "원", name]} />
            <Legend wrapperStyle={{ fontSize: 10 }} />
            <Line type="monotone" dataKey="gen_gmv" name="일반 CPS" stroke="#3498db" strokeWidth={2.5} dot={false} />
            <Line type="monotone" dataKey="yogi_gmv" name="yogithe" stroke="#e74c3c" strokeWidth={2.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      {vendorData.length > 0 && (
        <div style={{ background: "white", borderRadius: 10, padding: "16px", marginBottom: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: "#444", marginBottom: 4 }}>CPS 입점 매장 수 추이</div>
          <div style={{ fontSize: 10, color: "#bbb", marginBottom: 12 }}>일별 유니크 vendor_id 수 (CPS 클릭 기준)</div>
          {(() => {
            const filtered = filterByRange(vendorData, range);
            const latestGen = filtered[filtered.length - 1]?.general || 0;
            const latestYogi = filtered[filtered.length - 1]?.yogithe || 0;
            return (
              <>
                <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                  <div style={{ flex: 1, background: "#f8f9fd", borderRadius: 8, padding: "8px 12px" }}>
                    <div style={{ fontSize: 10, color: "#999" }}>일반 CPS 매장</div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: "#3498db" }}>{latestGen.toLocaleString("ko-KR")}개</div>
                  </div>
                  <div style={{ flex: 1, background: "#fdf8f8", borderRadius: 8, padding: "8px 12px" }}>
                    <div style={{ fontSize: 10, color: "#999" }}>요기더적립 매장</div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: "#e74c3c" }}>{latestYogi.toLocaleString("ko-KR")}개</div>
                  </div>
                  <div style={{ flex: 1, background: "#f8fdf8", borderRadius: 8, padding: "8px 12px" }}>
                    <div style={{ fontSize: 10, color: "#999" }}>요기더적립 비중</div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: "#22aa55" }}>{latestGen + latestYogi > 0 ? ((latestYogi / (latestGen + latestYogi)) * 100).toFixed(1) : 0}%</div>
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={filtered.map(r => ({ ...r, date: dateLabel(r.date) }))} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="date" tick={{ fontSize: 9 }} interval={xInterval(filtered.length)} />
                    <YAxis tickFormatter={v => v >= 10000 ? (v/10000).toFixed(0) + "만" : v.toLocaleString("ko-KR")} tick={{ fontSize: 9 }} width={45} />
                    <Tooltip formatter={(v, name) => [v.toLocaleString("ko-KR") + "개", name === "general" ? "일반 CPS" : "요기더적립"]} />
                    <Legend formatter={v => v === "general" ? "일반 CPS" : "요기더적립"} />
                    <Line type="monotone" dataKey="general" stroke="#3498db" strokeWidth={1.5} dot={false} />
                    <Line type="monotone" dataKey="yogithe" stroke="#e74c3c" strokeWidth={1.5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </>
            );
      ...
      {/* CPS 매장 카테고리 분포 (최근 7일) */}
      {categoryData.length > 0 && (
        <div style={{ background: "white", borderRadius: 10, padding: "16px", marginBottom: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: "#444", marginBottom: 4 }}>CPS 매장 카테고리 분포</div>
          <div style={{ fontSize: 10, color: "#bbb", marginBottom: 12 }}>최근 7일 CPS 클릭 기준 · 카테고리별 유니크 매장 수</div>
          <div style={{ display: "flex", gap: 16 }}>
            {["general", "yogithe"].map(ch => {
              const items = categoryData.filter(r => r.channel === ch).sort((a, b) => b.cnt - a.cnt).slice(0, 12);
              const maxCnt = items[0]?.cnt || 1;
              return (
                <div key={ch} style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: ch === "general" ? "#3498db" : "#e74c3c", marginBottom: 8 }}>
                    {ch === "general" ? "일반 CPS" : "요기더적립"}
                  </div>
                  {items.map((it, i) => (
                    <div key={it.category} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                      <div style={{ width: 14, fontSize: 10, color: "#999", textAlign: "right", flexShrink: 0 }}>{i + 1}</div>
                      <div style={{ width: 60, fontSize: 11, color: "#333", flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.category}</div>
                      <div style={{ flex: 1, background: "#f0f0f0", borderRadius: 3, height: 14, overflow: "hidden" }}>
                        <div style={{ width: (it.cnt / maxCnt * 100).toFixed(0) + "%", height: "100%", background: (ch === "general" ? "#3498db" : "#e74c3c") + "99", borderRadius: 3, transition: "width 0.3s" }} />
                      </div>
                      <div style={{ width: 50, fontSize: 10, color: "#666", textAlign: "right", flexShrink: 0 }}>{it.cnt.toLocaleString("ko-KR")}</div>
                    </div>
                  ))}
                </div>
              );
...
export default function YPXDashboard({ onClose }) {
  ...
  const [sigunguData, setSigunguData] = useState([]); // 시군구 전체
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (subResult.rows?.length > 0) {
        const fresh = subResult.rows.map(r => ({ date: r.date, classic: +r.classic, naver: +r.naver, toss: +r.toss, direct_ypx: +r.direct_ypx, new_naver: +r.new_naver || 0, new_toss: +r.new_toss || 0, new_direct: +r.new_direct || 0, new_classic: +r.new_classic || 0 }));
  ...
  const refreshRegion = useCallback(async () => {
    ...
    try {
      ...
      const [subResult, ordResult, sigunguResult] = await Promise.all([
        queryBigQuery(REGION_SUB_SQL(afterSub)),
        queryBigQuery(REGION_ORD_SQL(afterOrd)),
        queryBigQuery(REGION_SIGUNGU_SQL()),
      ]);
      ...
      if (sigunguResult.rows?.length) setSigunguData(sigunguResult.rows);
    ...
    try {
      ...
      if (result.rows?.length) {
        ...
        const allKws = [...new Set([
          ...Object.keys(merged[0] || {}).filter(k => k.startsWith('kw_') && k.endsWith('_search')).map(k => k.replace('kw_','').replace('_search','').normalize('NFC')),
          ...freshKws,
        ])];
    ...
    try {
      ...
      try {
        const [vendorResult, catResult] = await Promise.all([
          queryBigQuery(CPS_VENDOR_COUNT_SQL(afterDate)),
          queryBigQuery(CPS_CATEGORY_SQL(afterDate)),
        ]);
        if (vendorResult.rows?.length) {
          const vendorRows = vendorResult.rows.map(r => ({ date: r.date, channel: r.channel, vendor_cnt: +r.vendor_cnt }));
          // pivot: {date, general, yogithe}
          const vMap = {};
          for (const r of vendorRows) {
            if (!vMap[r.date]) vMap[r.date] = { date: r.date, general: 0, yogithe: 0 };
            vMap[r.date][r.channel] = r.vendor_cnt;
          }
          const vendorData = Object.values(vMap).sort((a, b) => a.date.localeCompare(b.date));
          saveCache(CPS_VENDOR_CACHE_KEY, vendorData);
          setCpsVendorData(vendorData);
        }
        if (catResult.rows?.length) {
          setCpsCategoryData(catResult.rows.map(r => ({ channel: r.channel, category: r.cat, cnt: +r.cnt })));
        }
      } catch (e) { console.warn("vendor/category query failed:", e.message); }
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 2000, display: "flex", alignItems: "flex-start", justifyContent: "flex-end" }}
      ...
      <div style={{ width: "min(1280px, 96vw)", height: "100vh", background: "#f4f6fb", display: "flex", flexDirection: "column", boxShadow: "-4px 0 24px rgba(0,0,0,0.12)", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
          ...
          {activeTab === "region" && (
            <RegionContent regionData={regionData} regionOrdData={regionOrdData} sigunguData={sigunguData} regionLoaded={regionLoaded} refreshStatus={regionRefreshStatus} onRefresh={refreshRegion} range={globalRange} />
          )}
          {activeTab === "age" && (
